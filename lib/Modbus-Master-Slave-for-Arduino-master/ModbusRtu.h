...
//Serial2 << "start addr=" << u8StartAdd <<  " regno=" << u8regsno << endl;
...
int8_t Modbus::process_FC5( uint16_t *regs, uint8_t u8size )
{
    ...
    u8currentRegister = (uint8_t) (u16coil / 16);
    u8currentBit = (uint8_t) (u16coil % 16);
//Serial2 << "u8currentRegister=" << u8currentRegister << " u8currentBit=" << u8currentBit << " value=" <<  (au8Buffer[ NB_HI ] == 0xff) << endl;
    // write to coil
    bitWrite(
